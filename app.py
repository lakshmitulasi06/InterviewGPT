...
with col_left:
    ...
    # 1. ALWAYS DISPLAY INTERVIEW DIALOGUE LOGS FIRST
    for message in st.session_state.chat_history:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

    # 2. RUN THE INTERVIEW LOOP PROCESS
    if st.session_state.vectordb and st.session_state.candidate_profile:
        
        # If no active question is presented, generate one immediately
        ...
        # 3. CAPTURE THE ANSWER WITHOUT AN st.form WRAPPER
        # Removing the form wrapper allows variables to commit to state natively on input
        user_answer = st.text_area(
            "Your Response:", 
            height=150, 
            placeholder="Type your full technical explanation here...",
            key="current_user_response_field"
        )
        ...
        if st.button("🚀 Submit Answer & Next Question"):
            if user_answer.strip() == "":
                ...
                # Instantly save the user's text into the running log history array
                ...
                if "error" not in evaluation:
                    ...
                    # Create the Markdown evaluation layout block
                    feedback_report = f"""
### ⚖️ Evaluation Report
...
*{evaluation.get('improved_answer')}*
                    """
                    # Save evaluation feedback to the chat logs
                    st.session_state.chat_history.append({"role": "assistant", "content": feedback_report})
                else:
                    st.error("Evaluation error encountered.")
                    
                # CLEAR the current question value out. 
                # On the next automatic line rerun, the code above will trigger a new question generation.
                st.session_state.current_question = None
                st.rerun()
